...
function normalize(page: Awaited<ReturnType<typeof queryDatabase>>[number]): EsteiraItem {
  ...
  return {
    ...
    status: (getSelect(properties, "Status") || "Em andamento") as EsteiraStatus,
    responsavel: getPeopleNames(properties, "Responsável"),
    retrabalho: getNumber(properties, "Retrabalho (%)"),
    percentualConcluido: getNumber(properties, "(%) Concluído"),
